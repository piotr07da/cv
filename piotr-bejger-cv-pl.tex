...
{	
	\cvEnuItemBox
	{
		\begin{itemize}
			...
			\item Znajomość: \textbf{DDD}, \textbf{CQRS}, \textbf{Event Sourcing}
			\item Znajomość: \textbf{.NET} (framework/core), WPF (MVVM), WCF, WF, ASP.NET
			\item Znajomość: \textbf{Azure DevOps}, \textbf{Git}
...
\cvSectionEntry
{
	...
	\cvSectionEntryBody{Wybrane osiągnięcia:}
	{
		\begin{itemize}
			...
			\item Stworzenie mechanizmów i narzędzi do ułatwienia zarządzania konfiguracją systemu zarówno na środowiskach developerskich jak i produkcyjnych. Wykorzystanie między innymi usługi Consul. Dzięki temu uzyskanie sprawniejszego i mniej ryzykownego deploymentu.
		\end{itemize}
	}
}
...
\cvSectionEntry
{
	...
	\cvSectionEntryBody{Wybrane osiągnięcia:}
	{
		\begin{itemize}
			...
			\item Projekt i implementacja dedykowanego mechanizmu logowania przebiegu procesów biznesowych, stosowanego do analizy wydajności i prawidłowości ich działania. (\textbf{C\#}, \textbf{.NET}, \textbf{MsSql})
			\item Integracja systemów zewnętrznych KRD, BIK, Asseco DEF z CasePro. (\textbf{WF}, \textbf{WCF})
			\item Stworzenie aplikacji do wysyłania JPK (jednolitych plików kontrolnych).
			\item Stworzenie aplikacji pozwalającej rejestrować zdarzenia komunikacyjne w Krakowie dla MPK Kraków. (\textbf{WPF}, \textbf{WCF})
		\end{itemize}
	}
	\vspace{43mm}
}
