...
{	
	\cvEnuItemBox
	{
		\begin{itemize}
			...
			\item Znajmość: \textbf{Azure, Kubernetes, Docker}.
			\item Znajomość: \textbf{Azure DevOps, Git}.
			\item Umiejętność ekstrakcji wiedzy domenowej z wykorzystanie techniki \textbf{Event Storming}.
...
\cvSectionEntry
{
	\cvSectionEntryHeader{❺}{Neuca (senior .NET developer)}{2021-04}{\cdot\cdot\cdot}

	\cvSectionEntryBody{Wybrane osiągnięcia:}
	{
		\begin{itemize}
			\setlength\itemsep{-1pt}
			\item Budowa podsystemu obsługi zamówień:
			\begin{itemize}
			\setlength\itemsep{-1pt}
				\item Ekstrakcja wiedzy domenowej przy pomocy techniki \textbf{Event Storming}.
				\item Projekt architektury rozwiązania w oparciu o zebraną wiedzę.
				\item Implementacja modułów systemu w oparciu o \textbf{DDD}.
				\item Implementacja biblioteki pozwalającej na zastosowanie \textbf{Event Sourcingu} w oparciu o dowolną bazę danych (np. EventStore, CosmosDb) oraz implementacja microframeworka umożliwiającego pisanie w prosty, deklaratywny sposób \textbf{testów jednostkowych} w modułach wykorzystujących Event Sourcing i DDD.
				\item Nadzorowanie prac, Code Review na przestrzeni całego projektu.
				\item Umożliwienie uruchomienia rozwiązania zarówno on-premise jak i w \textbf{Azure} z wykorzystaniem natywnych usług Azure.
				\item Wdrożenie systemu na Azure z wykorzystaniem \textbf{Kubernetes}.
			\end{itemize}
		\end{itemize}
	}
	\vspace{22pt}
...
\cvSectionEntry
{
	\cvSectionEntryHeader{❹}{ProService Finteco Sp. z o.o. (senior .NET developer / tech lead)}{2017-03}{2021-03}
...
\cvSectionEntry
{
	\cvEnuItemBox
	{
		\begin{itemize}
			...
			\item Amatorskie programowanie mikrokontrolerów AVR, STM32 bezpośrednio oraz na platoformach typu Arduino. Amatorskie IoT.
			...
			\item Zainteresowania: strzelectwo (aktywny członek International Defensive Pistol Association jako zawodnik, safety officer oraz sędzia), trójbój siłowy.
		\end{itemize}
	}
	\vspace{28pt}
}
